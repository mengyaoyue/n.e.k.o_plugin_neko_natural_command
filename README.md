...
- **v0.7.0**：管理面板重做（四页签 + 搜索筛选 + 复制 id + 界面偏好），i18n 补齐到 8 语言；`plugin.toml` 补 `[plugin.ui]`（宿主才有「打开界面」入口）。
...
在「插件管理 → 打开界面」进入（也可直接访问 `http://127.0.0.1:15680`）。面板由插件自己起的本地服务提供，只监听 `127.0.0.1`。

四个页签：

| 页签 | 有什么 |
|------|--------|
| **概览** | 状态卡片（命令数、可用能力、已装插件、自动学命令、密码状态）+ 警示条（密码未设置 / 模型不可用时会明确提示）+ 已安装插件列表 |
| **命令库** | 搜索（名称 / id / 描述）+ 类型 / 权限筛选 + 每条命令卡片（类型 / 权限 / 风险等级胶囊 + **一键复制 id** |
| **能力** | 当前所有插件暴露的入口，可搜索 + 复制 `entry id`（跨插件调用时最常要的东西） |
| **设置** | 自动学新命令开关、重载命令库 + 能力表、界面（字体 / 字号 / 鼠标轨迹）与模型 / 端口信息 |

细节：

- **命令行的 id 是跨插件引用的关键**（例如 `/调用 猫娘ID:命令id`），所以每条都给了复制按钮；
  浏览器禁用剪贴板时会自动帮用户选中，按 `Ctrl+C` 即可。
- 界面偏好（字体 / 字号 / 鼠标轨迹）存在 `data/panel_state.json` 的 `ui` 段（`/api/panel_prefs`）。
- 想换背景图：把一张 `bg.jpg` 放进插件的 `static/` 目录即可；没有就用默认渐变。
- 字体是自托管的（Quicksand / Lora / JetBrains Mono，共约 58KB），中文走系统字体栈；
  字号是按 `--fs` 派生的，切换时是整页缩放。

接口一览（都是本机 JSON）：

```text
GET  /api/status         状态 + 命令库 + 能力表 + 界面偏好
POST /api/reload         重载命令库与能力表
POST /api/toggle_auto_create  开关自动学命令
GET|POST /api/panel_prefs     界面偏好读写
```
